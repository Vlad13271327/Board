...
class Responses(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if title:
            ...
            context['filter_responses'] = list(Response.objects.filter(post_id=post_id).order_by('date_in'))
            context['response_post_id'] = post_id.id
        else:
            context['filter_responses'] = list(Response.objects.filter(post_id__author_id=self.request.user).order_by('date_in'))
        context['myresponses'] = list(Response.objects.filter(author_id=self.request.user).order_by('date_in'))
        return context
