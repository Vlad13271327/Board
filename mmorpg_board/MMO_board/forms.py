...
class PostForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['text'].label = 'Текст обьявления:'
...
class RespondForm(forms.ModelForm):
    class Meta:
        ...
        fields  =('text',)
    def __init__(self, *args, **kwargs):
        super(RespondForm, self, *args, **kwargs)
...
class ResponsesFilterForm(forms.Form):
    def __init__(self, user, *args, **kwargs):
        ...
        self.fields['title'] = forms.ModelChoiceField(
            label='Обьявление',
            queryset= Post.objects.filter(author_id=user.id).order_by('date_in').values_list('title', flat=True),
            empty_label='Все',
            required= False
        )